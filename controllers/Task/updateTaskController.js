...
const updateTask = async (req, res) => {
    ...
    if (isNaN(taskId)) {
        return res.status(400).json({ message: 'ID de la tâche invalide.' ,
            code: " INID-03",
            isError: true});
    }
    ...
    try {
        ...
        if (Array.isArray(todos)) {
            ...
            for (const todo of todos) {
                if (todo.idTodo) {
                    ...
                    await Todo.create({ ...todo, idTask: taskId });
                }
            }
        }
        ...
        res.status(200).json({ message: 'Tache mis à jour avec succès.', tasks, updatedTodos, isError: false });
    } catch (error) {
